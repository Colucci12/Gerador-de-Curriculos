"""Lê job_profile.md, renderiza HTML/CSS e gera PDF com WeasyPrint."""
...
def has_contact(contact: dict) -> bool:
    return any(contact.get(key) for key in ("email", "telefone", "cidade", "linkedin", "github"))


def render_pdf(md_path: Path, out_path: Path) -> None:
    text = md_path.read_text(encoding="utf-8")
    context = enrich_context_with_html(parse_markdown(text))
